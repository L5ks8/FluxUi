...
# Store them in a dict: { id: {"class": class_name, "parent": parent_id, "props": [(prop, val)], "methods": [(method, args)], "tags": [] } }
instances = {}
for match in re.finditer(r"G2L\[\"([a-zA-Z0-9]+)\"\] = Instance\.new\(\"([a-zA-Z0-9_]+)\"(?:, (G2L\[\"[^\"]+\"\]|game:GetService\(\"Players\"\)\.LocalPlayer:WaitForChild\(\"PlayerGui\"\)))?\);", text):
    id_ = match.group(1)
...
for match in re.finditer(r"G2L\[\"([a-zA-Z0-9]+)\"\]\[\"([a-zA-Z0-9_]+)\"\] = (.*?);", text):
    ...
    if id_ in instances:
        ...
        if prop == "Name":
            # Extract string content from val (e.g., [[name]] or "name")
...
for id_, data in instances.items():
    ...
    # Clean base name to valid lua identifier
